...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, needresize, issticky;
  pid_t pid;
	Client *next;
	Client *snext;
  Client *swallowing;
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, w - 2 * sp, bh, lrpad, m->sel->name, 0);
			if (m->sel->isfloating)
				drw_rect(drw, x + boxs + lrpad / 2, boxs,
